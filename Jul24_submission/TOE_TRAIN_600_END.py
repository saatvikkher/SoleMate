...
sys.stdout = open('./TOE_TRAIN_600_END.log', 'w')

print("[TOE TRAIN] Started.")
...
for i in range(len(combined_test)):
    start = time.time()
    if i % 20 == 0:
        print("[TOE TRAIN] Progress: ", (i*100) / len(combined_test))
    try:
        row = pd.DataFrame(process_image(
            Q_files[i], K_files[i], mated[i], partial_type="toe"), index=[0])
        df = pd.concat([df, row], ignore_index=True)
        df.to_csv("RESULT_TOE_TRAIN_600_END.csv", index=False)
    except Exception as e:
        print("[TOE TRAIN] Caught error at index " + str(i) + str(e))
    end = time.time()
    print("[TOE TRAIN]: Time for iteration ", str(i), ": ", str(end - start))
    gc.collect()

print("[TOE TRAIN] Complete!")
