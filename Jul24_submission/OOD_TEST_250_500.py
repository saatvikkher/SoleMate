...
sys.stdout = open('./OOD_TEST_250_500.log', 'w')

print("[TEST OOD] Started.")

# Read in training split
km_test = pd.read_csv("Jul24_submission/pair_info/OOD_TEST_KM.csv")
knm_test = pd.read_csv("Jul24_submission/pair_info/OOD_TEST_KNM.csv")
combined_test = pd.concat([km_test, knm_test], ignore_index=True)[250:500]
...
for i in range(len(combined_test)):
    ...
    try:
        ...
        df.to_csv("RESULT_OOD_TEST_250_500.csv", index=False)
