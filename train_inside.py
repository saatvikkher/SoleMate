...
sys.stdout = open('./train_inside.log', 'w')

print("[TRAIN INSIDE] Started.")
...
for i in range(len(combined_train)):
    start = time.time()
    if i % 20 == 0:
        print("[TRAIN INSIDE] Progress: ", (i*100) / len(combined_train))
    try:
        row = pd.DataFrame(process_image(
            Q_files[i], K_files[i], mated[i], partial_type="inside"), index=[0])
        df = pd.concat([df, row], ignore_index=True)    
        df.to_csv("result_train_inside_0707.csv", index=False)
    except Exception as e:
        print("[TRAIN INSIDE] Caught error at index " + str(i) + str(e))
    end = time.time()
    print("[TRAIN INSIDE]: Time for iteration ", str(i), ": ", str(end - start))
    gc.collect()

print("[TRAIN INSIDE] Complete!")
