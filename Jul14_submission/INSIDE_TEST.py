...
sys.stdout = open('./INSIDE_TEST.log', 'w')
...
for i in range(len(combined_test)):
    ...
    try:
        ...
        df.to_csv("RESULT_INSIDE_TEST.csv", index=False)
    except Exception as e:
        print("[INSIDE TEST] Caught error at index " + str(i) + str(e))
    end = time.time()
    print("[INSIDE TEST]: Time for iteration ", str(i), ": ", str(end - start))
    gc.collect()

print("[INSIDE TEST] Complete!")
