...
sys.stdout = open('./DIFF_TIME_1VS2_TEST_250_END.log', 'w')

print("[DIFF TIME 1VS2 TEST] Started.")

# Read in testing split
km_test = pd.read_csv("Jul24_submission/pair_info/DIFF_TIME_1VS2_TEST_KM.csv")
knm_test = pd.read_csv("Jul24_submission/pair_info/DIFF_TIME_1VS2_TEST_KNM.csv")
...
for i in range(len(combined_test)):
    start = time.time()
    if i % 20 == 0:
        print("[DIFF TIME 1VS2 TEST] Progress: ", (i*100) / len(combined_test))
    try:
        row = pd.DataFrame(process_image(
            Q_files[i], K_files[i], mated[i]), index=[0])
        df = pd.concat([df, row], ignore_index=True)
        df.to_csv("RESULT_DIFF_TIME_1VS2_TEST_250_END.csv", index=False)
