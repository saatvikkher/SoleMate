...
sys.stdout = open('./OUTSIDE_TRAIN_600_END.log', 'w')

print("[OUTSIDE TRAIN] Started.")
...
for i in range(len(combined_test)):
    start = time.time()
    if i % 20 == 0:
        print("[OUTSIDE TRAIN] Progress: ", (i*100) / len(combined_test))
    try:
        row = pd.DataFrame(process_image(
            Q_files[i], K_files[i], mated[i], partial_type="outside"), index=[0])
        df = pd.concat([df, row], ignore_index=True)
        df.to_csv("RESULT_OUTSIDE_TRAIN_600_END.csv", index=False)
    except Exception as e:
        print("[OUTSIDE TRAIN] Caught error at index " + str(i) + str(e))
    end = time.time()
    print("[OUTSIDE TRAIN]: Time for iteration ", str(i), ": ", str(end - start))
    gc.collect()

print("[OUTSIDE TRAIN] Complete!")
