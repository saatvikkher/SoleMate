...
sys.stdout = open('./test_inside.log', 'w')

print("[TEST INSIDE] Started.")
...
for i in range(len(combined_test)):
    start = time.time()
    if i % 20 == 0:
        print("[TEST INSIDE] Progress: ", (i*100) / len(combined_test))
    try:
        row = pd.DataFrame(process_image(
            Q_files[i], K_files[i], mated[i], partial_type="inside"), index=[0])
        df = pd.concat([df, row], ignore_index=True)    
        df.to_csv("result_test_inside_0707.csv", index=False)
    except Exception as e:
        print("[TEST INSIDE] Caught error at index " + str(i) + str(e))
    end = time.time()
    print("[TEST INSIDE]: Time for iteration ", str(i), ": ", str(end - start))
    gc.collect()

print("[TEST INSIDE] Complete!")
