...
class SolePairCompare:
    ...
    def _cluster_prop_metric(self, df_a: pd.DataFrame, df_b: pd.DataFrame, n_clusters: int) -> float:
        ...
        for i in range(n_clusters):
            ...
            cluster_prop_diff.append(num_in_cluster_a / len_a - num_in_cluster_b / len_b)
    ...
    def cluster_metrics(self, n_clusters:int = 20):
        hcluster_centroids = self._hierarchical_cluster(self.Q_coords, n_clusters = n_clusters)
        ...
        metrics_dict = {}
        metrics_dict['centroid_distance'] = self._centroid_distance_metric(q_kmeans_centroids, k_kmeans_centroids)
        metrics_dict['cluster_proprtion'] = self._cluster_prop_metric(q_df_labels, k_df_labels, n_clusters)
        metrics_dict['iterations_q'] = q_kmeans.n_iter_
        metrics_dict['iterations_k'] = k_kmeans.n_iter_
        metrics_dict['wcv'] = self._within_cluster_var_metric(q_df_labels, k_df_labels, q_kmeans_centroids, k_kmeans_centroids, n_clusters)
