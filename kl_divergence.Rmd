...
overlap_proportion <- function(df1, df2){
  df1_den <- density(df1, from = min(df1), to = max(df1))
  df2_den <- density(df2, from = min(df2), to = max(df2))
 
  joint <- pmin(df1_den$y, df2_den$y)
  
  overlap <- sum(joint) / sum(df1_den$y, df2_den$y)
...
for (filename in filenames){
  ...
  for (col in cols[15:52]){
    ...
    overlap2 <- overlap_proportion(nonmated, mated)
    df <- bind_rows(df, as.tibble(col, kld1, kld2, kst, overlap, overlap2))
  }
