...
class SolePair():
    ...
    def _downsample(self) -> np.ndarray | np.ndarray:
    ...
    def icp_transform(self, max_iterations=10000, tolerance=0.00001) -> pd.DataFrame | pd.DataFrame:
        q_pts, k_pts = self._downsample()

        T,_,i = icp(q_pts,k_pts, max_iterations=max_iterations, tolerance=tolerance)
        transformed_q = pd.DataFrame(self._transform(q_pts, T))
        transformed_q = transformed_q.rename(columns = {0: 'x', 1 : 'y'})
        self.Q.aligned = transformed_q
        return transformed_q, self.K.coords
