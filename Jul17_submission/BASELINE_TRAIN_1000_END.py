...
sys.stdout = open('./BASELINE_TRAIN_1000_END.log', 'w')

print("[BASELINE TRAIN 1000 - end] Started.")
...
combined_train = pd.concat([km_train, knm_train], ignore_index=True)[1000:]
...
for i in range(len(combined_train)):
    start = time.time()
    if i % 20 == 0:
        print("[BASELINE TRAIN 1000 - end] Progress: ", (i*100) / len(combined_train))
    try:
        row = pd.DataFrame(process_image(
            Q_files[i], K_files[i], mated[i]), index=[0])
        df = pd.concat([df, row], ignore_index=True)
        df.to_csv("RESULT_BASELINE_TRAIN_1000_END.csv", index=False)
    except Exception as e:
        print("[BASELINE TRAIN 1000 - end] Caught error at index " + str(i) + str(e))
    end = time.time()
    print("[BASELINE TRAIN 1000 - end]: Time for iteration ", str(i), ": ", str(end - start))
    gc.collect()

print("[TRAIN BASELINE 1000 - end] Complete!")
